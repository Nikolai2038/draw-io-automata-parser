...
function array_set() {
  ...
  if ((args_amount < 2)); then
    print_error "At least two arguments are required! The last value should be the assigned value for the variable, followed by the keys. Number of arguments received: ${C_COMMAND}${args_amount}${C_RETURN}" || return "$?"
  fi
